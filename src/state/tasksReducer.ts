import {addNewTodoAC, removeTodoListAC, setTodolistsAC} from "./todoListReducer";
...
export const tasksReducer = (state = initialState, action: ActionsType): TaskStateType => {
    switch (action.type) {
        ...
        // case ACTIONS_TYPE.EDIT_TASK_TITLE:
        //     return {
        //         ...state,
...
export const removeTaskAC = (todoID: string, taskID: string) => {
    return {type: ACTIONS_TYPE.REMOVE_TASK, todoID, taskID} as const;
}
...
export const updateTaskAC = (todoID: string, taskID: string, model: UpdateTaskModelType) => {
    return {type: ACTIONS_TYPE.UPDATE_TASK, todoID, taskID, model} as const;
}
export const setTasksAC = (todoID: string, tasks: TaskType[]) => {
    return {type: ACTIONS_TYPE.SET_TASKS, todoID, tasks} as const;
}
//Thunk creators
export const getTasks = (todoID: string) => async (dispatch: AppDispatch) => {
    const {data} = await tasksAPI.getTasks(todoID);
    dispatch(setTasksAC(todoID, data.items));
}
export const deleteTask = (todoID: string, taskID: string) => async (dispatch: AppDispatch) => {
...
export const createTask = (todoID: string, title: string) => async (dispatch: AppDispatch) => {
...
export const updateTask = (todoID: string, taskID: string, domainModel: UpdateTaskModelType) => async (dispatch: AppDispatch, getState: () => AppRootStateType) => {
